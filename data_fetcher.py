...
def fetch_taiex(date_str=None):
    ...
    優先使用 TWSE 舊版 FMTQIK (?date=YYYYMMDD)，回傳該月所有交易日資料，拿最後一筆 = 最近交易日。
    若失敗回退到 OpenAPI FMTQIK 與 MI_INDEX。
    """
    print("撈取 大盤指數...")
    if date_str is None:
    ...
    # 主要來源: TWSE 舊版 FMTQIK，往前回溯最多 10 天直到拿到資料
    ...
    for back in range(10):
        ...
        print(f"  [legacy FMTQIK {try_date}] index={idx_val} change={result['change']} (date row={latest[0]})")
        break

    # 備援1: TWSE OpenAPI FMTQIK
        ...
        try:
            ...
            if resp_oa:
                ...
                if isinstance(oa_data, list) and len(oa_data) >= 1:
                    ...
                    print(f"  [OpenAPI FMTQIK fallback] index={result['index']} change={result['change']}")
        except Exception as e:
...
def fetch_all_data(date_str=None):
    ...
    ★ 國際指標使用快取累積機制：
      每次執行時，先載入前次快取，與新抓取的數據合併後存回。
      即使當次 FRED/Stooq 全部失敗，仍可保留前幾天的歷史數據。
      跑 7 天後就保證有 7+ 個數據點（至少近7個交易日）。
    """
    if date_str is None:
        date_str = datetime.now().strftime("%Y%m%d")
    ...
    jpy_merged = _merge_and_trim(
        history_cache.get("jpy_rate", []),
        jpy_fresh,
        35
    )
    ...
    data = {
        ...
        "usd_index": usd_merged[-30:],
        "jpy_rate": jpy_merged[-30:],
        "vix": vix_result,
